...
<?php
require_once '../models/Galeriadestino.php';
...
if(isset($_REQUEST['action']))
{
	...
	switch($action)
	{
		case 'registrar':
			...
			echo "<meta http-equiv ='refresh' content='0;url=galeriadestino.php?idd=". urlencode(encripta($idd,'rayedgard'))."&n=".urlencode(encripta($nombre,'rayedgard'))." '>"; 	
			/*/------------FIN ACCIONES-----------------/*/
			break;
		...
		case 'eliminar':
			...
			if($nombreFoto!=null or $nombreFoto!="")
				unlink("../images/galeriaDestino/".$nombreFoto); //elimina en la carpeta
			break;
					...
					<h3 class="blank1"><?php echo $titulo;?></h3>
						...
						<div class="tab-pane active" id="horizontal-form" >
							<div class="bs-example4" data-example-id="contextual-table">
								...
								<form class="form-horizontal" action="galeriadestino.php?action=<?php echo urlencode(encripta("registrar","rayedgard")); ?>&idd=<?php echo urlencode(encripta($idd,'rayedgard'));?>&n=<?php echo urlencode(encripta($nombre,'rayedgard'));?>" method="post" enctype="multipart/form-data">
						...
						<div class="bs-example4" data-example-id="contextual-table">
							<table class="table">
							  ...
							  <tbody>
								...
								<?php 
								$cont=1;
								foreach($galeria->Listar($idd) as $r): ?>
			                        <tr>
			                        	<td><?php echo $cont++; ?></td>
			                            <td><?php echo $r->__GET('nombre'); ?></td>
			                        	 <td>
			                       			<img src="../images/galeriaDestino/<?php echo $r->__GET('nombre');  ?>" height="30" width="30"/> 
			                             </td>
                                      ...
                                      <a href="#"  onclick="javascript:direc('?action=<?php echo urlencode(encripta('eliminar','rayedgard'));?>&id=<?php echo urlencode(encripta($r->id,'rayedgard')); ?>&idd=<?php echo urlencode(encripta($idd,'rayedgard')); ?>&n=<?php echo urlencode(encripta($nombre,'rayedgard')); ?>&nn=<?php echo $r->__GET('nombre'); ?>','<?php echo $r->__GET('nombre'); ?>')" class="btn btn-sm btn-danger">Eliminar</a>
