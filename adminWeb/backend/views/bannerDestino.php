...
<?php
require_once '../models/Bannerdestino.php';
...
// Logica
$bannerdestino = new Bannerdestino();
$titulo = "Gesti&oacute;n de Banners";
...
if(isset($_REQUEST['action']))
{
	...
	switch($action)
	{
		case 'registrar':
			...
			$bannerdestino->__SET('titulo',     $_REQUEST['titulo']);
			$bannerdestino->__SET('detalle',     $_REQUEST['detalle']);
			...
			{ 
				echo "La extensión o el tamaño del archivo de IMAGEN no es correcta. <br><br><table><tr><td><li>Se permiten archivos *.gif, *.png o *.jpg<br><li>se permiten archivos de 3Mb maximo.</td></tr></table><br>";
				  ...
				  $bannerdestino->__SET('foto', $nombre_archivo1);
			...
			$bannerdestino->__SET('destino_id', $idd);

			
			$bannerdestino->Registrar($bannerdestino);
			//header('Location: index.php');
			/*/------------PARA LAS ACCIONES-----------------/*/ 			
			
			echo "<meta http-equiv ='refresh' content='0;url=bannerDestino.php?idd=". urlencode(encripta($idd,'rayedgard'))."&n=".urlencode(encripta($nombre,'rayedgard'))." '>"; 	
					/*/------------FIN ACCIONES-----------------/*/
			break;
		...
		case 'eliminar':
			...
			$bannerdestino->Eliminar($id);//elima en la base de datos

			//header('Location: index.php');
			if($nombreFoto!=null or $nombreFoto!="")
				unlink("../images/bannerDestino/".$nombreFoto); //elimina en la carpeta
			break;
					...
					<h3 class="blank1"><?php echo $titulo;?></h3>
						...
						<div class="tab-pane active" id="horizontal-form" >
							<div class="bs-example4" data-example-id="contextual-table">
								...
								<form class="form-horizontal" action="bannerDestino.php?action=<?php echo urlencode(encripta("registrar","rayedgard")); ?>&idd=<?php echo urlencode(encripta($idd,'rayedgard'));?>&n=<?php echo urlencode(encripta($nombre,'rayedgard'));?>" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<div class="col-sm-8">
												...
												<input type="text" name="titulo" value="<?php echo $bannerdestino->__GET('titulo'); ?>" class="form-control1" id="focusedinput" placeholder="Escriba un titulo" title="Escriba un titulo" required="">										
										...
										<div class="form-group">
											...
											<div class="col-sm-8">
												...
												<!--CAMBIO EN name y value-->
												<input type="text" name="detalle" value="<?php echo $bannerdestino->__GET('detalle'); ?>" class="form-control1" id="focusedinput" placeholder="Escriba el detalle" title="Escriba el detalle" required="">										
										...
										<div class="form-group">
											...
											<div class="col-sm-4">
												<input  type="file" name="foto" id="exampleInputFile" value="<?php echo $bannerdestino->__GET('nombre'); ?>" >												
						...
						<div class="bs-example4" data-example-id="contextual-table">
							<table class="table">
							  ...
							  <tbody>
								...
								<?php 
								$cont=1;
								foreach($bannerdestino->Listar($idd) as $r): ?>
			                        <tr>
			                        	<td><?php echo $cont++; ?></td>
			                            <td><?php echo $r->__GET('titulo'); ?></td>
			                            <td><?php echo $r->__GET('detalle'); ?></td>
			                        	 <td>
			                       			<img src="../images/bannerDestino/<?php echo $r->__GET('foto');  ?>" height="30" width="30"/> 
			                             </td>
                                      ...
                                      <a href="#"  onclick="javascript:direc('?action=<?php echo urlencode(encripta('eliminar','rayedgard'));?>&id=<?php echo urlencode(encripta($r->id,'rayedgard')); ?>&idd=<?php echo urlencode(encripta($idd,'rayedgard')); ?>&n=<?php echo urlencode(encripta($nombre,'rayedgard')); ?>&nn=<?php echo $r->__GET('foto'); ?>','<?php echo $r->__GET('titulo'); ?>')" class="btn btn-sm btn-danger">Eliminar</a>
