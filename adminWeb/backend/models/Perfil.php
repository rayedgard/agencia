...
class Perfil
{
    ...
    public function buscarPerfil($id)
    {
        ...
        try
        {
            ...
            $stm = $this->pdo->prepare('SELECT p.id,p.nombre,p.cargo,p.telefono,p.correo,p.foto,p.detalle,p.idioma_id,p.estado FROM perfil p WHERE p.estado=1 and p.id ='.$id);
            $stm->execute();
